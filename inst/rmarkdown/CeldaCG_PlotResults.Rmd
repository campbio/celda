...
```{r setup, eval = TRUE, echo = showSetup}
...
if(!is.null(cellAnnot)) {
  if(!all(cellAnnot %in% colnames(colData(altExp(sce, e = altExpName))))) {
    s <- setdiff(cellAnnot, colnames(colData(altExp(sce, e = altExpName))))
    stop("The following items in 'cellAnnotations' were not found in the ",
         "column data of the object: ", paste0(s, ","))
  }
}
...
if(!is.null(cellAnnot)) {
  for (i in cellAnnot) {
    cat(sprintf(tab4, i))
    print(plotSCEDimReduceColData(altExp(sce), sample=sce$sample,
                                  colorBy = i, reducedDim = reducedDimName,
                                  labelClusters = FALSE))
    cat(space)
  }  
} else {
  message("No cell annotations are displayed. To plot cell annotations in this section, you can pass the desired variables from the `colData` in the SingleCellExperiment object via the 'cellAnnot' parameter.")
} 
...
```{r plot_markers, eval=plotMarkers}

ix <- retrieveSCEIndex(inSCE = sce, IDs = features, axis = "row",
                       by = displayName,
                       exactMatch = exactMatch)
if(length(ix) > 0) {
  print(plotDimReduceFeature(sce, reducedDimName = "celda_UMAP",
                           features = rownames(sce)[ix],
                           colorHigh = "red", colorMid = "grey",
                           colorLow = "blue", ncol = 3))
} else {
