...
plotGridSearchPerplexity.celda_CG <- function(celdaList, sep) {
    ...
    if (is.null(perplexity(celdaList))) {
        stop("No perplexity measurements available. First run",
            " 'resamplePerplexity' with celdaList object.")
    }

    ix1 <- rep(seq(nrow(perplexity(celdaList))),
        each = ncol(perplexity(celdaList)))
    ix2 <- rep(seq(ncol(perplexity(celdaList))), nrow(perplexity(celdaList)))
    df <- data.frame(runParams(celdaList)[ix1, ],
        perplexity = perplexity(celdaList)[cbind(ix1, ix2)])
    df$K <- as.factor(df$K)
    df$L <- as.factor(df$L)
...
plotGridSearchPerplexity.celda_G <- function(celdaList, sep) {
    ...
    if (length(perplexity(celdaList)) == 0) {
        stop("No perplexity measurements available. First run",
            " 'resamplePerplexity' with celdaList object.")
    }
