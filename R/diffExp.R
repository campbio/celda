...
#'  cells. This matrix should be the same as the one used to generate `celda.mod`.
...
#' @param only.pos Logical. Whether to only return markers with positive log2 
#'  fold change. Default FALSE.
#' @param log2fc.threshold Numeric. A number greater than 0 that specifies the 
#'  absolute log2 fold change threshold. Only features with absolute value above 
#'  this threshold will be returned. If NULL, this filter will not be applied. 
#'  Default NULL.
#' @param fdr.threshold Numeric. A number between 0 and 1 that specifies the 
...
differentialExpression <- function(counts, 
    ...
    if (!(methods::is(celda.mod, "celda_C") || 
        methods::is(celda.mod, "celda_CG"))){
    ...
    if (length(c1) > 1){
        cells1 <- celda.mod@names$column[which(celda.mod@clusters$z %in% c1)]
    } else {
        cells1 <- celda.mod@names$column[which(celda.mod@clusters$z == c1)]
    }
    if (length(c2) > 1){
        cells2 <- celda.mod@names$column[which(celda.mod@clusters$z %in% c2)]
    } else {
        cells2 <- celda.mod@names$column[which(celda.mod@clusters$z == c2)]
    }
    mat <- counts[, c(cells1,cells2)]
    logNormalizedMat <- normalizeCounts(mat, normalize="cpm", 
        transformation.fun=log1p)
    ...
    sca <- suppressMessages(MAST::FromMatrix(logNormalizedMat, cdat))
    ...
    # clusters have 0 counts for a gene and then they're discarded 
    # because NaN !> 0
    if (is.null(log2fc.threshold)) {
        fcHurdleSig <-fcHurdle  
    } else {
        fcHurdleSig <- merge(fcHurdle[fdr < fdr.threshold &
                           abs(coef) > log2fc.threshold], 
            data.table::as.data.table(GenomicRanges::mcols(sca)), by = 'primerid')
        if (only.pos) {
