...
differentialExpression <- function(counts, 
    ...
    log_normalized_mat <- normalizeCounts(mat, normalize = "cpm", 
    ...
    summaryCond <- MAST::summary(zlmCond, doLRT = "conditionc2")
    summaryDt <- summaryCond$datatable
    
    # Avoid NSE notes in check
    contrast <- component <- primerid <- coef <- ci.hi <- ci.lo <- NULL 
    `Pr(>Chisq)` <- fdr <- NULL
    
    fcHurdle <- merge(summaryDt[contrast == "conditionc1" & component == "H", 
          .(primerid, `Pr(>Chisq)`)],
    ...
    if (is.null(log2fcThreshold)) {
        ...
        fcHurdleSig <- merge(fcHurdle[fdr < fdrThreshold & abs(coef) > log2fcThreshold], 
