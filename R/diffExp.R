...
differentialExpression <- function(counts,
    ...
    if (length(c1) > 1) {
        cells1 <- matrixNames(celdaMod)$column[which(clusters(celdaMod)$z %in% c1)]
    } else {
        cells1 <- matrixNames(celdaMod)$column[which(clusters(celdaMod)$z == c1)]
    }

    if (length(c2) > 1) {
        cells2 <- matrixNames(celdaMod)$column[which(clusters(celdaMod)$z %in% c2)]
    } else {
        cells2 <- matrixNames(celdaMod)$column[which(clusters(celdaMod)$z == c2)]
    }
