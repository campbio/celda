...
.calCellPct <- function(counts, z, geneMarkers, threshold = 1) {
    ...
    if (length(intersectedGenes) == 0) {
        stop("Cannot find marker genes in 'counts'. Make sure the name of the",
            " marker genes in 'geneMarker' consistent with row-names in",
            " 'count'.")
    }
...
.vctrRowProjectColCollapse <- function(sub_vector, genePresented, z,
    ...
    if (length(CTnames) > 1) {
        ...
        pct_CTbyZ <- matrix(rep(pct_CTbyZ, length(CTnames)),
            nrow = length(CTnames), dimnames = list(CTnames, seq(K)))
    } else {
        # Only one cell type has this marker
        pct_CTbyZ <- matrix(pct_CTbyZ, nrow = 1, dimnames = list(CTnames,
            seq(K)))
    }
    return(pct_CTbyZ)
}
