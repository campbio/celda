...
#' @param  z a vector. Cell cluster label, same length as the column numbers of
#'  the count matrix
#' @param  geneMarkers a dataframe of marker genes which corresponding to their
#'  cell types
#' @param  threshold default as 1.
#' @param  color default as "red3"
#' @param  textLabelSize default as 3
#' @param  precision default as 2. Precision of percentage of cells showing the
#'  marker gene shown on the barplot
#' @export
decontXMarkerPlot <- function(counts, z, geneMarkers, threshold = 1,
    color = "red3", textLabelSize = 3, precision = 2) {
    ...
    pct_CTbyZ.melt <- reshape2::melt(pct_CTbyZ, varnames = c("cellType", "z"),
        value.name = "percent")

    plt <- ggplot2::ggplot(pct_CTbyZ.melt, ggplot2::aes(x = z,
        y = pct_CTbyZ.melt$percent * 100)) +
        ggplot2::geom_bar(stat = "identity", fill = color) +
        ggplot2::geom_text(aes(x = z, y = pct_CTbyZ.melt$percent * 100 + 5,
            label = paste0(round(pct_CTbyZ.melt$percent, precision) * 100,
                "%")), size = textLabelSize) +
        ggplot2::xlab("Cluster") +
        ggplot2::ylab(paste0("Percentage of cells expressing",
            " cell-type\nspecific markers")) +
...
.calCellPct <- function(counts, z, geneMarkers, threshold = 1) {
    ...
    if (length(intersectedGenes) == 0) {
        stop("Cannot find marker genes in 'counts'. Make sure the name of",
            " the marker genes in 'geneMarker' consistent with row-names in",
            " 'count'.")
    }
    ...
    geneMarkers <- .geneMarkerProcess(geneMarkers,
        orders = rownames(sub_counts))

    if (is.null(nrow(sub_counts))) {
        # When there is only one gene --> a vector
        pct_CTbyZ <- .vctrRowProjectColCollapse(sub_vector = sub_counts,
            genePresented = geneMarkers, z = z, threshold = threshold)
    } else {
        # When multiple genes --> matrix
        pct_CTbyZ <- .mtxRowProjectColCollapse(sub_counts = sub_counts,
            genePresented = geneMarkers, z = z, threshold = threshold)
    }
...
.vctrRowProjectColCollapse <- function(sub_vector, genePresented, z,
    ...
    pct_CTbyZ <- sweep(binary_2byZ, MARGIN = 2, STATS = table(z),
        FUN = "/")[rownames(binary_2byZ) == TRUE, ]

    if (length(CTnames) > 1) {
        # Multiple cell types have this marker
        pct_CTbyZ <- matrix(rep(pct_CTbyZ, length(CTnames)),
            nrow = length(CTnames), dimnames = list(CTnames, 1:K))
    } else {
        # Only one cell type has this marker
        pct_CTbyZ <- matrix(pct_CTbyZ, nrow = 1,
            dimnames = list(CTnames, seq(K)))
    }
    return(pct_CTbyZ)
}
...
.mtxRowProjectColCollapse <- function(sub_counts, genePresented, z,
    ...
    if (nrow(genePresented) == length(unique(genePresented[,
        "geneMarkers"]))) {
        ...
        # TODO: make sure there is no same gene same celltype pair has shown
        # more than once

        duplicatedMarker <- genePresented[duplicateTF, ]

        for (r in seq(nrow(duplicatedMarker))) {
    ...
    ng_CTbyC <- Matrix::sparseMatrix(i = i_celltype, j = iC, x = 1,
        giveCsparse = TRUE, dimnames = list(CTnames, Cnames),
        dims = c(nTC, ncol(sub_counts)))
...
# geneMarkers should be a dataframe,  w/ 2 column names being `cellType` and
# `geneMarkers`
# convert both `cellType` and `geneMarkers` are factors with levels being
#  integer
.geneMarkerProcess <- function(geneMarkers, orders = None) {
    ...
    levels(geneMarkers[, "cellType"]) <- seq(length(levels(geneMarkers[,
        "cellType"])))
...
# geneMarkers = data.frame( cellType = c(rep("Tcells", 3), rep("Bcells", 3),
#    "DC"), geneMarkers = 1:7) # string as factor
