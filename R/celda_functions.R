...
#' @param celda.mod A single celda model as generated by celda(), of class "celda_C", "celda_G", "celda_CG"
...
#' @export
compareCountMatrix = function(count.matrix, celda.mod, error.on.mismatch=TRUE) {
  if (length(celda.mod$y != 0) & nrow(count.matrix) != length(celda.mod$y)) {
    stop("The provided celda object was generated from a counts matrix with a different number of genes than the one provided.")
  }
  
  if (length(celda.mod$z != 0 ) & ncol(count.matrix) != length(celda.mod$z)) {
