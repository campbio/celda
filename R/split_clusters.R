...
cC.splitZ = function(counts, m.CP.by.S, n.G.by.CP, n.CP, s, z, K, nS, nG, alpha, beta, z.prob, max.clusters.to.try=10, min.cell=3) {
  ...
  clust.split = lapply(z.to.split, function(x){
    clusters(.celda_C(counts[,z == x], K=2, z.initialize="random", max.iter=5, split.on.iter=-1, split.on.last=FALSE, verbose=FALSE))$z
  })
  
  clust.split = vector("list", K)
  for(i in z.to.split) { 
    clustLabel = .celda_C(counts[,z == i], K=2, z.initialize="random", max.iter=5, split.on.iter=-1, split.on.last=FALSE, verbose=FALSE)
...
cCG.splitY = function(counts, y, m.CP.by.S, n.G.by.CP, n.TS.by.C, n.TS.by.CP, n.by.G, n.by.TS, nG.by.TS, n.CP, s, z, K, L, nS, nG, alpha, beta, delta, gamma, y.prob, max.clusters.to.try=10, K.subclusters=10, min.cell=3) {
  ...
  for(i in z.non.empty) { 
    ...
    if(z.ta[i] <= K.subclusters) {
      ...
      clustLabel = .celda_C(counts[,z == i], K=K.subclusters, z.initialize="random", max.iter=5, split.on.iter=-1, split.on.last=FALSE, verbose=FALSE)
  ...
  for(i in y.to.split) { 
    clustLabel = .celda_G(temp.n.G.by.CP[y == i,], L=2, y.initialize="random", max.iter=5, split.on.iter=-1, split.on.last=FALSE, verbose=FALSE)
...
cG.splitY = function(counts, y, n.TS.by.C, n.by.TS, n.by.G, nG.by.TS, nM, nG, L, beta, delta, gamma, y.prob, min.feature=3, max.clusters.to.try=10) { 
  ...
  clust.split = vector("list", L)
  for(i in y.to.split) {
    clustLabel = .celda_G(counts[y == i,], L=2, y.initialize="random", max.iter=5, split.on.iter=-1, split.on.last=FALSE, verbose=FALSE)
    clust.split[[i]] = clustLabel@clusters$y
  }
