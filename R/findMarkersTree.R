...
findMarkersTree <- function(features,
  ...
  if (methods::hasArg(celda)) {
    ...
    #get feature labels
    featureLabels <-
      unlist(apply(seurat@reductions$pca@feature.loadings, 1,
                   function(x)
  ...
  if (!methods::hasArg(metaclusters) & (!autoMetaclusters)) {
    ...
    tree <- .generateTreeList(features,
                              class,
                              oneoffMetric,
                              threshold,
                              reuseFeatures,
    ...
    subtypeLabels[subtypeLabels %in% largeMetaclusters] <-
      paste0(subtypeLabels[subtypeLabels %in% largeMetaclusters],
             "(",
      ...
      newTree <- list(
        rules = .mapClass2features(newTree,
                                   features[metaclusterLabels
                                            == metacluster, ],
                                   as.factor(subtypeLabels[metaclusterLabels == metacluster]))$rules,
    ...
    branchPoints <- lapply(branchPoints, function(br) {
      ...
      br$class[grepl("\\(.*\\)", br$class)] <- regmatches(br$class[grepl("\\(.*\\)", br$class)],
                                                          regexpr(pattern = "(?<=\\().*?(?=\\)$)",
    ...
    tree$rules <-
      suppressWarnings(lapply(tree$rules, function(r) {
        ...
        r$class[grepl("\\(.*\\)", r$class)] <- regmatches(r$class[grepl("\\(.*\\)", r$class)],
                                                          regexpr(pattern = "(?<=\\().*?(?=\\)$)",
...
.createBranchPoints <-
  function(rules, largeMetaclusters, metaclusters) {
    ...
    branchPoints <- lapply(branchPoints, function(level) {
      ...
      if (setequal(level[level$feature == firstFeat &
...
.generateTreeList <- function(features,
  ...
  treeLevel[[1]] <- .wrapSplitHybrid(features,
                                     class,
                                     threshold,
                                     oneoffMetric)
  
  # Add set of features used at this split
  treeLevel[[1]]$fUsed <- unlist(lapply(treeLevel[[1]][names(treeLevel[[1]]) != "statUsed"],
                                        function(X) {
                                          X$featureName
                                        }))
      ...
      lapply(treeLevel, function(split, features, class) {
        ...
        if (length(split) == 4 &
          ...
          if (!is.null(branch1)) {
            ...
            branch1$fUsed <- c(split$fUsed, unlist(lapply(branch1[names(branch1) != "statUsed"],
                                                          function(X) {
                                                            X$featureName
                                                          })))
        ...
        if (length(split) == 4 &
          ...
          if (!is.null(branch2)) {
            # Add feature to list of used features.
            branch2$fUsed <- c(split$fUsed, unlist(lapply(branch2[names(branch2) != "statUsed"],
                                                          function(X) {
                                                            X$featureName
                                                          })))
          ...
          group1Samples <- unique(unlist(lapply(split[!names(split) %in% c("statUsed", "fUsed", "dirs")],
                                                function(X) {
                                                  X$group1
                                                })))
          group2Samples <- unique(unlist(lapply(split[!names(split) %in% c("statUsed", "fUsed", "dirs")],
                                                function(X) {
                                                  X$group2
                                                })))
          ...
          if (length(group2Classes) > 1) {
            ...
            if (!is.null(branch2)) {
              ...
              branch2$fUsed <-
                c(split$fUsed, unlist(lapply(branch2[names(branch2) != "statUsed"],
                                             function(X) {
                                               X$featureName
                                             })))
...
.wrapSplitHybrid <- function(features,
  ...
  splitList <- lapply(names(splitStats),
                      .getSplit,
                      splitStats,
                      features,
                      class,
  ...
  if (length(splitList) > 1) {
    ...
    splitList <- lapply(unique(group1Vec),
                        function(group1, splitList, group1Vec) {
                          ...
                          splitFeature <- unlist(lapply(splitListSub,
                                                        function(X) {
                                                          X$featureName
                                                        }))
                          splitValue <- unlist(lapply(splitListSub,
                                                      function(X) {
                                                        X$value
                                                      }))
                          splitStat <- unlist(lapply(splitListSub,
                                                     function(X) {
                                                       X$stat
                                                     }))
  ...
  names(splitList) <- unlist(lapply(splitList,
                                    function(X) {
...
.splitMetricModF1 <-
  function(feat, class, features, rPerf = FALSE) {
    ...
    keep <- c(classSort[seq(1, (len - 1))] != classSort[seq(2, (len))] &
  ...
  function(feat, class, features, rPerf = FALSE) {
    ...
    if (sum(sKeep) > 0) {
      ...
      if (nrow(classSplitUnique) > 0) {
        ...
        names(IGdens) <- apply(classSplitUnique * 1,
                               1,
                               function(X) {
                                 paste(X, collapse = "")
                               })
...
.addAlternativeSplit <- function(tree, features, class) {
  ...
  groupList <- lapply(DecTree, function(split) {
    ...
    # Get groups
    group1 <- unique(unlist(lapply(split,
                                   function(node) {
                                     node$group1Consensus
                                   })))
    group2 <- unique(unlist(lapply(split,
                                   function(node) {
  ...
  if (length(AltSplitInd) > 0) {
    ...
    altStats <- do.call(rbind,
                        lapply(colnames(fSub),
                               function(feat,
                                        splitMetric,
                                        features,
                                        class,
    ...
    altSplit <- .getSplit(altStats$feat[1],
                          splitStats,
                          fSub,
                          cSub,
                          .splitMetricModF1)
    ...
    if (length(altSplit$group1Consensus) == 1) {
      ...
      downSplit <- downSplit[c(which(!names(downSplit) %in% c("statUsed", "fUsed", "dirs")),
      ...
      if (wBranch == 1) {
        ...
        wSplit <- which(seq((branchCum[(wBranch - 1)] + 1),
      ...
      unlist(lapply(classes, function(cl, rules, level, samp) {
        ...
        if (sum(ruleClass$direction == 1) > 1) {
          ruleClass <- ruleClass[order(ruleClass$direction
                                       , decreasing = T),]
          ruleClass <- ruleClass[c(which.max(ruleClass$stat[ruleClass$direction == 1]),
...
.convertToDendrogram <- function(tree, class, splitNames = NULL) {
  ...
  statRep <- unlist(lapply(DecTree,
                           function(split) {
                             length(split[!names(split) %in% c("statUsed", "fUsed", "dirs")])
                           }))
  statUsed <- unlist(lapply(seq(length(statUsed)),
                            function(i) {
  ...
  for (i in seq(1, length(DecTree))) {
    ...
    if (length(split) == 1) {
      ...
      group1classUnique <- unique(lapply(split,
                                         function(X) {
                                           X$group1Consensus
                                         }))
      ...
      group2classUnique <- unique(unlist(lapply(split,
                                                function(X) {
                                                  X$group2Consensus
                                                })))
      group2classUnique <-
...
.mapClass2features <-
  function(tree, features, class, topLevelMeta = FALSE) {
    ...
    class2featuresIndices <- do.call(rbind, lapply(seq(length(tree)),
                                                   function(i) {
                                                     treeLevel <- tree[[i]]
                                                     c2fsub <- as.data.frame(do.call(rbind, lapply(treeLevel,
                                                                                                   function(split) {
                                                                                                     ...
                                                                                                     edgeFram <-
                                                                                                       do.call(rbind, lapply(split, function(edge) {
                                                                                                         ...
                                                                                                         sdir <- c(rep(1, length(edge$group1Consensus)),
...
plotMarkerDendro <- function(tree,
  ...
  dendSegsAlt <- dendSegs[dendSegs$x != dendSegs$xend |
...
.highlightClassLabel <- function(dendro, classLabel) {
  ...
  while (flag) {
    ...
    wSplit <- which(unlist(lapply(labList,
                                  function(vec) {
                                    classLabel %in% vec
                                  })))
...
plotMarkerHeatmap <-
  function(tree,
    ...
    if (branchPoint == "top_level") {
      ...
      colOrder <- data.frame(groupName = names(sort(
        table(tree$metaclusterLabels), decreasing = T
      )),
    ...
    if (branch$statUsed[1] == "Split") {
      ...
      cellAnno <-
        data.frame(split = rep("Down-regulated", ncol(counts)),
    ...
    if (branch$statUsed[1] == "One-off") {
      ...
      colOrder <- data.frame(groupName = unique(branch[order(branch$direction, decreasing = T), "class"]),
