...
#            (model_DecontXoneBatch$run.params$delta + colSums(model_DecontXoneBatch$res.list$est.rmat) ) / ( 2*model_DecontXoneBatch$run.params$delta + colSums(Decon.sim$cmat + Decon.sim$rmat) ) )
    ...
    test_that( desc = "Testing logLikelihood.DecontXoneBatch", {
        ...
        expect_equal( decon.calcLL(counts=Decon.sim$cmat+Decon.sim$rmat, z=z.process  ,  
        ...
        expect_equal( bg.calcLL( counts=Decon.sim$cmat+Decon.sim$rmat, theta=model_DecontXoneBatchbg$res.list$theta, 
    ...
    test_that( desc = "Testing decontamination EM updates", {
        z.process = processCellLabels(Decon.sim$z, num.cells=ncol(Decon.sim$rmat) )
        expect_equal( cD.calcEMDecontamination( counts=Decon.sim$cmat+Decon.sim$rmat, z=z.process, K=length(unique(Decon.sim$z)),
