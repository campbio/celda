    ...
    test_that( desc = "Testing logLikelihood.DecontXoneBatch", {
        ...
        expect_equal( decon.calcLL(omat=Decon.sim$cmat+Decon.sim$rmat, z=z.process  ,  
        ...
        expect_equal( bg.calcLL( omat=Decon.sim$cmat+Decon.sim$rmat, theta=model_DecontXoneBatchbg$res.list$theta, 
    ...
    test_that( desc = "Testing decontamination EM updates", {
        z.process = processCellLabels(Decon.sim$z, num.cells=ncol(Decon.sim$rmat) )
        expect_equal( cD.calcEMDecontamination( omat=Decon.sim$cmat+Decon.sim$rmat, z=z.process, K=length(unique(Decon.sim$z)),
