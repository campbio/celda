...
modelCG <- celda_CG(counts=celdaCGSim$counts, 
    sample.label=celdaCGSim$sample.label, K=celdaCGSim$K, L=celdaCGSim$L, 
    algorithm="EM", verbose=FALSE)
factorized <- factorizeMatrix(celda.mod = modelCG, counts = celdaCGSim$counts)
...
test_that(desc = "Testing compareCountMatrix with numeric matrix input", {
  # Case from GitHub issue #137
    ...
    expect_true(compareCountMatrix(counts, modelCG, error.on.mismatch = TRUE))
})

test_that(desc = "Testing appendCeldaList", {
    egList <- celda::celda.CG.grid.search.res
    expect_error(appendCeldaList(egList, matrix(0)),
    ...
    expect_warning(appendCeldaList(egList, modifiedEgList),
        "Provided lists have different count.checksums and may have", 
        "been generated from different count matrices. Using checksum",
        " from first list...")
    expect_equal(length(egList@res.list)*2, 
        length(appendCeldaList(egList, egList)@res.list))
})
