...
model_CG = celda_CG(counts = celdaCG.sim$counts,
    ...
    algorithm = "EM",
    verbose = FALSE)
factorized <- factorizeMatrix(celda.mod = model_CG,
...
test_that(desc = "Testing simulation and celda_CG model", {
    ...
    expect_true(all(sweep(factorized$counts$cell,
        2,
        colSums(celdaCG.sim$counts),
        "/") == factorized$proportions$cell))
    expect_equal(celdaCG.sim$K,
        ncol(factorized$proportions$cell.population))
    expect_equal(celdaCG.sim$L,
        nrow(factorized$proportions$cell.population))
    expect_true(all(is.numeric(
        logLikelihoodHistory(celda.mod = model_CG))))
    expect_equal(max(logLikelihoodHistory(celda.mod = model_CG)),
        bestLogLikelihood(model_CG))
...
test_that(desc = "Testing clusterProbability with celda_CG", {
    ...
    expect_true(all(round(rowSums(clust.prob$z.probability), 10) == 1) &
            nrow(clust.prob$z.probability) == ncol(celdaCG.sim$counts))
    expect_true(all(round(rowSums(clust.prob$y.probability), 10) == 1) &
            nrow(clust.prob$y.probability) == nrow(celdaCG.sim$counts))

    clust.prob = clusterProbability(counts = celdaCG.sim$counts,
        model_CG,
        log = TRUE)
    expect_true(all(round(rowSums(normalizeLogProbs(clust.prob$z.probability)),
            10) == 1) &
        nrow(clust.prob$z.probability) == ncol(celdaCG.sim$counts))
    expect_true(all(round(rowSums(normalizeLogProbs(clust.prob$y.probability)),
            10) == 1) &
        nrow(clust.prob$y.probability) == nrow(celdaCG.sim$counts))
})
...
test_that(desc = "Testing simulateCells.celda_CG, make sure all genes expressed", {
    sim.cells.low <- simulateCells(model = "celda_CG",
        G = 1000,
        C = 300,
        C.Range = c(1, 100),
        N.Range = c(1, 100))
...
test_that(desc = "Testing celdaGridSearch with celda_CG", {
    ...
    expect_warning(
        celdaGridSearch(
            ...
            params.fixed = list(z.initialize = "random", y.initialize = "random")
        ),
        "Parameter 'nchains' should not be used within the params.test list"
    )


    celdaCG.res <- celdaGridSearch(
        counts = celdaCG.sim$counts,
        model = "celda_CG",
        nchains = 2,
    ...
    celdaCG.res = resamplePerplexity(celdaCG.sim$counts, celdaCG.res, resample =
            2)
    expect_is(celdaCG.res, "celdaList")
    expect_error(resamplePerplexity(celdaCG.sim$counts, celdaCG.res, resample =
            "2"))
    expect_error(resamplePerplexity(celdaCG.sim$counts, "celdaCG.res", resample =
            2))
    ...
    expect_error(
        subsetCeldaList(celda.list = "celda_list"),
    ...
    expect_error(selectBestModel("celda_list"),
        "celda.list parameter was not of class celdaList.")
    expect_error(celdaCG.res <-
            resamplePerplexity(celdaCG.sim$counts, model_CG, resample = 2))
    expect_error(
        celdaCG.res <-
            resamplePerplexity(celdaCG.sim$counts, celdaCG.res, resample = 'a')
    )

    celdaCG.res.index1 = subsetCeldaList(celdaCG.res, params = list(index = 1))
    expect_true(all(
        is(celdaCG.res.index1, "celda_CG") &&
            !is(celdaCG.res.index1, "celda_list")
    ))

    res <- perplexity(celdaCG.sim$counts, model_CG)
    res2 <-
        perplexity(celdaCG.sim$counts, model_CG, new.counts = celdaCG.sim$counts + 1)
...
test_that(desc = "Testing normalizeCounts with celda_CG", {
    ...
    expect_error(
        normalizeCounts(celdaCG.sim$counts, transformationFun = "scale"),
        "'transformationFun' needs to be of class 'function'"
    )
    expect_error(
        normalizeCounts(celdaCG.sim$counts, scaleFun = "scale"),
...
test_that(desc = "Testing CompareCountMatrix with celda_CG", {
    expect_true(compareCountMatrix(counts = celdaCG.sim$counts, celdaMod = model_CG))

    less.features <- celdaCG.sim$counts[1:50, ]
    expect_error(
        compareCountMatrix(counts = less.features, celda.mod = model_CG),
    ...
    expect_error(
        compareCountMatrix(counts = less.cells, celda.mod = model_CG),
    ...
    counts.matrix.error <-
        matrix(
            data = 1,
            nrow = nrow(celdaCG.sim$counts),
            ncol = ncol(celdaCG.sim$counts)
        )
    expect_false(
        compareCountMatrix(
            counts = counts.matrix.error,
            celda.mod = model_CG,
...
test_that(desc = "Testing topRank with celda_CG", {
    top.rank <-
        topRank(
            matrix = factorized$proportions$module,
            n = 1000,
            threshold = NULL
        )
    expect_equal(names(top.rank), c("index", "names"))
    top.rank <-
        topRank(matrix = factorized$proportions$module, n = 1000)
...
test_that(desc = "Testing plotHeatmap with celda_CG", {
    expect_error(
        plotHeatmap(
            counts = celdaCG.sim$counts,
            z = model_CG@clusters$y,
            y = model_CG@clusters$y
        ),
    ...
    expect_error(
        plotHeatmap(
            counts = celdaCG.sim$counts,
            z = model_CG@clusters$z,
            y = model_CG@clusters$z
        ),
    ...
    expect_error(
        ...
        "'scale.row' needs to be of class 'function'"
    )
    expect_error(
        plotHeatmap(
            counts = celdaCG.sim$counts,
        ...
        "'trim' should be a 2 element vector specifying the lower and upper boundaries"
    )
    expect_equal(names(
        plotHeatmap(
            counts = celdaCG.sim$counts,
...
test_that(desc = "Testing plotHeatmap with annotations", {
    annot.cell <-
        as.data.frame(c(rep(
            x = 1, times = ncol(celdaCG.sim$counts) - 100
        ), rep(x = 2, 100)))
    annot.feature <-
        as.data.frame(c(rep(
            x = 1, times = nrow(celdaCG.sim$counts) - 100
        ), rep(x = 2, 100)))
    ...
    expect_equal(names(
        plotHeatmap(
            celda.mod = model_CG,
            counts = celdaCG.sim$counts,
            annotation.cell = annot.cell,
            annotation.feature = annot.feature,
            z = model_CG@clusters$z,
            y = model_CG@clusters$y
        )
    ),
    ...
    expect_equal(names(
        plotHeatmap(
            celda.mod = model_CG,
            counts = celdaCG.sim$counts,
            annotation.cell = as.matrix(annot.cell),
            annotation.feature = as.matrix(annot.feature),
            z = model_CG@clusters$z,
            y = model_CG@clusters$y
        )
    ),
...
test_that(desc = "Testing celdaHeatmap with celda_CG", {
    expect_equal(names(
        celdaHeatmap(celda.mod = model_CG, counts = celdaCG.sim$counts)
    ),
        c("tree_row", "tree_col", "gtable"))
})
...
test_that(desc = "Checking moduleHeatmap to see if it runs", {
    ...
    expect_error(
        ...
        "'celda.mod' should be an object of class celda_G or celda_CG"
    )
})

# celdaProbabilityMap
test_that(desc = "Testing celdaProbabiltyMap.celda_CG for sample level", {
    plot.obj = celdaProbabilityMap(counts = celdaCG.sim$counts,
        celda.mod = model_CG,
        level = "sample")
...
test_that(desc = "Testing differentialExpression for celda_CG", {
    ...
    expect_error(
        ...
        "'celda.mod' should be an object of class celda_C or celda_CG"
    )
    expect_error(
        differentialExpression(
            counts = celdaCG.sim$counts,
    ...
    expect_equal(
        names(
            plotDimReduceCluster(
                dim1 = celda.tsne[, 1],
                dim2 = celda.tsne[, 2],
                cluster = as.factor(model_CG@clusters$z),
...
test_that(desc = "Testing celdaTsne.celda_CG with all cells", {
    tsne = celdaTsne(
        counts = celdaCG.sim$counts,
        celda.mod = model_CG,
        max.cells = length(model_CG@clusters$z)
    )
    plot.obj = plotDimReduceCluster(tsne[, 1], tsne[, 2], model_CG@clusters$z)
    expect_true(ncol(tsne) == 2 &
            nrow(tsne) == length(model_CG@clusters$z))
    expect_true(!is.null(plot.obj))

    tsne = celdaTsne(
        counts = celdaCG.sim$counts,
        celda.mod = model_CG,
        max.cells = ncol(celdaCG.sim$counts),
        modules = 1:2
    )
    expect_error(
        tsne <-
...
test_that(desc = "Testing celdaUmap.celda_CG with all cells", {
    ...
    expect_true(ncol(umap) == 2 &
            nrow(umap) == length(model_CG@clusters$z))
    expect_true(!is.null(plot.obj))
...
test_that(desc = "Testing perplexity.celda_CG", {
    ...
    expect_error(
        perplexity.celda_CG(celdaCG.sim$counts, model_CG),
        "could not find function \"perplexity.celda_CG\""
    )
})

test_that(desc = "Testing featureModuleTable", {
    table = featureModuleTable(celdaCG.sim$counts, model_CG, output.file = NULL)
    expect_equal(ncol(table), 10)
})

test_that(desc = "Testing violinPlot", {
    violin = violinPlot(counts = celdaCG.sim$counts,
        celdaMod = model_CG,
        features = "Gene_1")
    expect_equal(
        names(violin),
...
test_that(desc = "Invoking error from sample labels function", {
    expect_error(
        .processSampleLabels("Sample_1", ncol(celdaCG.sim$counts)),
        paste0(
            "'sampleLabel' must be the same length as the number of columns",
