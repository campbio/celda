...
Celda has a heatmap function that allows for semisupervised clustering.  One can specify the cluster labeles obtained from the model and within the cluster it will perform hierarchical clustering.

```{r celda_heatmap_toy_1,fig.width=7,fig.height=5 ,	out.width="0.6\\linewidth", fig.align = "center"}
celda_heatmap(counts = sample.cells, z = toy_celda_c$res.list[[1]]$z, z.trim=NULL)
```
...
```{r}
celda_cluster_assignments = celda( sim_counts$counts, model="celda_C",
...
celda_heatmap(counts = sim_counts$counts, z = estimated.z)
```
