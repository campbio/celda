...
> **Note:** As of celda 1.23.0, the DecontX algorithm is maintained in the
> standalone [decontX](https://bioconductor.org/packages/decontX/) package. The
> `decontX`, `plotDecontX*`, and `simulateContamination` functions in celda are
> thin wrappers around it, so the [decontX](https://bioconductor.org/packages/decontX/)
> package must be installed (`BiocManager::install("decontX")`) to run the code
> in this vignette.
